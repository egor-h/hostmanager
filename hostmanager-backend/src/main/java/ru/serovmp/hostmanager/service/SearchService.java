...
public class SearchService {
    ...
    public BriefSearchResultDto briefSearch(String query) {
        return BriefSearchResultDto.builder()
                .hosts(hostRepository.topRecents(query, SEARCH_LIMIT).stream().map(h -> new BriefSearchResultDto.BriefHost(h.getId(), h.getName(), h.getAddress())).collect(Collectors.toList()))
                .notes(noteRepository.topRecents(query, SEARCH_LIMIT).stream().map(note -> new BriefNoteDto(note.getId(), note.getTitle(), note.getCreatedAt(), note.isDone())).collect(Collectors.toList()))
                .protocols(protocolRepository.topRecents(query, SEARCH_LIMIT).stream().map(proto -> new ProtocolDto(proto.getId(), proto.getName(), proto.getExecutionLine(), proto.getLaunchType().name(), proto.getValidationRegex(), proto.getExpectedExitCode())).collect(Collectors.toList()))
                .tags(tagRepository.topRecents(query, SEARCH_LIMIT).stream().map(tag -> new TagDto(tag.getId(), tag.getName())).collect(Collectors.toList()))
                .build();
    }
}

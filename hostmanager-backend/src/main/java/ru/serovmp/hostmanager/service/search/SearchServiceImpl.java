...
public class SearchServiceImpl implements SearchService {
    ...
    public SearchServiceImpl(HostRepository hostRepository, ProtocolRepository protocolRepository, TagRepository tagRepository, HostServiceImpl hostService, NoteService noteService, Searcher searcher, List<Indexable> indexables) {
    ...
    public List<BriefSearchResultDto.BriefHost> searchHosts(String query, int page) {
        return hostRepository.textSearchByHostNameOrAddressPagable(query, PageRequest.of(page, SEARCH_PAGE_SIZE)).stream()
                .map(host -> new BriefSearchResultDto.BriefHost(host.getId(), host.getName(), host.isDir(), host.getAddress()))
                .collect(Collectors.toList());
    }
}
